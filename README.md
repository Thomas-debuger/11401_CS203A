...
樹狀類型,實際應用案例 ,為何適合（核心特性） 
二元樹 (Binary Tree),運算式樹 (Expression Tree),能自然地表示運算子與左右運算元的階層關係，方便進行數學或邏輯運算評估 。
完全二元樹 (Complete Binary Tree),堆積儲存 (Heap Storage),結構緊密且節點排列連續，能完美映射到陣列 (Array) 中，空間利用率極高 。
二元搜尋樹 (BST),字典與符號表,具備排序性質，在資料量適中且相對平衡時，能快速進行鍵值的搜尋、插入與刪除 。
AVL 樹,資料庫索引 (Database Indexing),高度平衡規則極其嚴格，能確保搜尋時間穩定維持在 O(logn)，適合大量查詢、極少更新的情境 。
紅黑樹 (Red-Black Tree),程式語言函式庫 (如 C++ Map/Set),平衡限制較寬鬆，插入與刪除時的旋轉次數較少，在頻繁更新的操作下效能最為穩定 。
最大堆積 (Max Heap),優先權排程 (Priority Scheduling),能在 O(1) 時間內取得最大值（優先權最高者），並在 O(logn) 內完成結構調整 。
最小堆積 (Min Heap),Dijkstra 最短路徑演算法,能快速取得最小值，是路徑搜尋與任務排程演算法中的核心組件 。
