...
* **Assignment III — Linked List Selection Sort**   
  這份作業延續了前一次的選擇排序主題，但將資料結構從陣列轉移到「**鏈結串列（Linked List）**」，重點在於比較兩者在演算法實作上的差異與效能表現。
  本次作業的核心內容：
  **鏈結串列的基礎與實作**
    * **節點結構繪製**：練習繪製單向鏈結串列（Singly Linked List）的節點，包含資料欄位（Value）與指標欄位（Next Pointer） 。
    * **資料初始化**：將給定的 8 個整數（60, 24, 15, 42, 20, 11, 90, 8）存入鏈結串列，並標註 `head`（頭指標）與 `tail`（尾節點） 。
    * **數值交換邏輯**：作業要求在排序時僅交換節點內的「數值（Value）」，而不修改節點間的指標指向，以簡化實作難度 。
  **2. 演算法執行追蹤**：詳細記錄前三個步驟的執行狀況。
    * **第一步**：從頭開始找尋最小值 8，並與第一個節點（60）交換 。
    * **後續步驟**：依序移動 `i` 指標，在剩餘的未排序節點中尋找最小值並進行交換 。
  **3. 陣列與鏈結串列的深度比較 (作業重點)**
...
* **Assignment IV — Hash Function Implementation**
  ...
  * 比較各種雜湊函數在不同輸入下的分布情形
  * 撰寫解說文件與示例程式碼
  * 整份作業以 **C 和 C++ 語言**完成，並在 Visual Studio Code 環境中編譯測試。
