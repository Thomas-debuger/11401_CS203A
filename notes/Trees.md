...
* **節點 v 的 height = 從 v 到最遠 leaf 的邊數**
...
# 1. Full Binary Tree（滿二元樹）= Strict Binary Tree（嚴格二元樹）
...
### 重要的前提：可合併性 (Associative Property)
...
#### 1. 嚴格二元樹 (Strict Binary Tree)

* **定義:** 樹中**所有非葉節點 (Non-leaf nodes)** 都**恰好有兩個子節點**。
* **特性:** 每個節點要麼是葉節點（0 個子節點），要麼是非葉節點（2 個子節點）。
* **範例 (如圖所示):**
    * 節點 1 (根節點) 有 2 個子節點 (2 和 3)。
    * 節點 2 有 2 個子節點 (4 和 5)。
    * 節點 3, 4, 5 是葉節點 (0 個子節點)。

#### 2. 完整二元樹 (Complete Binary Tree)

* **定義:** 除了**最底層**外，所有層級都是**完全填滿**的，並且最底層的節點都**盡可能靠左**排列。
* **特性:** 這種樹在用陣列（Array）表示時非常高效，因為節點之間沒有空隙。
* **範例 (如圖所示):**
    * 第 0 層 (節點 1) 是滿的。
    * 第 1 層 (節點 2, 3) 是滿的。
    * 第 2 層只缺了最右邊的一個節點 (節點 7 的位置)。節點 4、5、6 靠左連續排列。

#### 3. 退化二元樹 (Degenerate Binary Tree)

* **定義:** 樹中**所有節點**都**只有一個子節點**。
* **特性:** 這種樹的形狀與**鏈結串列 (Linked List)** 相同。
* **後果:** 這是二元樹的**最壞情況 (Worst Case)**。在這種情況下，樹的高度等於節點數 $n$，原本 $O(\log n)$ 的搜尋效率會**退化成 $O(n)$**，失去二元樹的優勢。
* **範例 (如圖所示):**
    * 節點 1 只有右子節點 2。
    * 節點 2 只有右子節點 3。
    * 節點 3 只有右子節點 4。

#### 4. 完美二元樹 (Perfect Binary Tree)

* **定義:** 樹的**所有內部節點**都**有兩個子節點**，並且**所有葉節點**都在**同一層級**。
* **特性:**
    * 它是最嚴格的二元樹，同時滿足**嚴格二元樹**和**完全二元樹**的特性。
    * 在高度為 $h$ 的完美二元樹中，節點總數 $N = 2^{h+1} - 1$。
* **範例 (如圖所示):**
    * 樹高 $h=2$ (從 0 開始算)。
    * 所有層級 (0, 1, 2) 都完全填滿。
    * 葉節點 (4, 5, 6, 7) 都在第 2 層。
