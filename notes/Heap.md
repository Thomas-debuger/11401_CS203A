...
## `build_heap(array)` — 把一個無序陣列 array 變成 Heap
...
## `extract-max / extract-min`（sift-down）— O(log n)

### 在做什麼？

* 移除最高 priority 的元素（root）

### 怎麼修？

* 用最後一個節點補 root
* 一路往下跟 child 比（sift-down）
...
* 底層節點多，但幾乎不用動 (大多數節點都在底層)
* 只有少數上層節點會動很多
* 總成本 = O(n)

👉 比 n 次 insert（O(n log n)）快

### 補充

**Bottom-up Heapify（必背）**    
*  從 n/2 − 1 開始
*  一路做到 index 0
*  每次用 sift-down
*  建 Heap 的時間是 O(n)
...
* 用 Heap 來排序
* **Heap 是一棵完全二元樹**，高度大約是 **log n**。
* Heapsort 主要在做一件事：**把最大值丟到最後面**，然後把堆修好。

### 怎麼做？

1. 先把資料變成 heap，build-heap → O(n)（這步大約 **O(n)**，不影響最後結果）
2. 接著做 **n 次** extract-max / min，每次 O(log n)：

   * 取出最大值（根）放到尾端
   * 然後「往下調整」讓它重新變成 heap
   * 這個「往下調整」最多走樹高，所以是 **O(log n)**
...
在 **heapsort（用陣列做 max-heap）** 的排序階段，通常是**往下調整**（sift-down / heapify-down），不是往上。

為什麼是往下？

* 你把「根(最大值)」跟「最後一個元素」交換後
  **新的根** 可能變小、違反 max-heap（根要最大）
* 這時候要把它一路跟「較大的子節點」交換，讓它下沉回正確位置
  → 這就是 **往下調整**

什麼時候是往上調整？

* 你在 **插入一個新元素** 到 heap 尾端時（priority queue 常見）
  新元素可能比父節點大，就要一路往上浮
  → 這才是 **往上調整（sift-up）**

所以：

* **建堆（自底向上建堆）**：每個節點做的是「往下調整」
* **排序階段每次交換根與尾**：也是「往下調整」

結論：heapsort 典型版本靠的是 **down-heap**，不是 up-heap。

---

# 二、考前「超精簡筆記版」（直接背）

## Heap Operations — 考前速記
...
* **increase-key（Max）**：O(log n)
  → sift-up

* **decrease-key（Max）**：O(log n)
  → sift-down

* **build-heap**：O(n)
  → bottom-up heapify（Floyd）
