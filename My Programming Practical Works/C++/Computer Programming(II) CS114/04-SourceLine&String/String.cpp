...
string& string::operator=(const string& right)
{
    if (this != &right)
    {
        if (right.myData.mySize > myData.myRes)
        {
            ...
            myData.bx.ptr = new value_type[myData.myRes + 1]();//?1.myData.myPtr() = new value_type[myData.myRes];
        }
...
string& string::erase(size_t off, size_t count)
{
    if (off < myData.mySize)
    {
        ...
        {
            for (size_t i = off; i < myData.mySize - count; i++) {
...
string string::substr(size_type off, size_type count) const
{
    ...
    {
        //string a(*this);//?2.為甚麼寫string a;會錯，因為沒有初始化大小給它
        string a(count, '\0');
        int num = 0;
        for (int i = off; i < off + count; i++) {
            a[num] = myData.myPtr()[i];
            num++;
        }
        a.myData.mySize = count;
        a.myData.myPtr()[count] = value_type();
        return a;
    }
}
