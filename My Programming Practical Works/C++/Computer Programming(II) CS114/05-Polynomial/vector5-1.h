...
public:
    ...
    iterator insert(const_iterator where, const value_type& val)
    {
        ...
        if (originalSize == originalCapacity)
        {
            ...
            return myData.myFirst + count;
        }
        else
        {
            for (int i = originalSize; i > where - myData.myFirst; i--) {
    ...
    iterator erase(const_iterator where)
    {
        ...
        if (where >= myData.myFirst || where <= myData.myLast) {
            for (int i = where - myData.myFirst; i < size(); i++) {
                myData.myFirst[i + 1] = myData.myFirst[i];
            }
            myData.myFirst--;
        }
