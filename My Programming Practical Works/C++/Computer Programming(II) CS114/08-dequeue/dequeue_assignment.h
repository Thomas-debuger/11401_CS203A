        ...
        if (&right != this) // avoid self-assignment
        {
            ...
            {  // copy data from right to the current object
                for (int i = 0; i < myData.mySize; i++) {
                    int a = (myData.myOff + i) % (dequeSize * myData.mapSize);
                    int b = (right.myData.myOff + i) % (dequeSize * right.myData.mapSize);
                    if (myData.map[a / dequeSize] == nullptr) {
                        myData.map[a / dequeSize] = new Ty[dequeSize]();
                    }
                    myData.map[a / dequeSize][a % dequeSize] = right.myData.map[b / dequeSize][b % dequeSize];
                }
            }
        }
